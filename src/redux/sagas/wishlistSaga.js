...
export function* addToWishlist(action) {
    ...
    try {
      ...
      const res = yield call(performPostRequestAddToWhishlist, path, action.payload.data);
...
export function* removeWishlist(action) {
  ...
  try {
      // yield put(commonActions.startSpinner());
      const  res = yield call(performPostRequestRemoveFromWhishlist,path, action.payload.data);
