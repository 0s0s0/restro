...
export const userLogin = function* userLoginSaga(action) {
  ...
  let Phone_Email =
    action.payload.typePE == "email"
      ? action.payload.user.login
      : COUNTRY_CODE + action.payload.user.login;
  ...
  try {
    ...
    if (res !== undefined && res.status === 200) {
      yield put({ type: CONST.USER_LOGIN_ACTION, id: res?.data?.data.id });
...
export const userForgot = function* userForgotSaga(action) {
  ...
  let phone_email =
    action.payload.type === "email"
      ...
      : COUNTRY_CODE + action.payload.email_phone;
